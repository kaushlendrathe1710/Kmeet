...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/turn-credentials", (req, res) => {
    ...
    console.log("[TURN] 📥 Request for TURN credentials received");
    console.log("[TURN] 🔧 Config loaded:", {
      url: turnServerUrl ? "✓ Present" : "✗ Missing",
      username: turnUsername ? "✓ Present" : "✗ Missing",
      password: turnPassword ? "✓ Present (hidden)" : "✗ Missing",
    });

    if (!turnServerUrl || !turnUsername || !turnPassword) {
      console.warn(
        "[TURN] ⚠️ No custom TURN server configured, using fallback STUN only"
      );
    ...
    console.log(`[TURN] ✅ Serving TURN server: turn:${hostPort}`);
    ...
    const iceServers: RTCIceServer[] = [
      ...
      // Primary: Domain-based TURN server (UDP)
      {
        urls: `turn:${hostPort}`,
    ...
    // Add TURNS (TLS) if port 5349 is likely configured
    ...
    if (port === "3478" || !port) {
      ...
      console.log(`[TURN] 🔒 Added TURNS on port 5349 for TLS connections`);
    }

    console.log(`[TURN] 📤 Sending ${iceServers.length} ICE servers to client`);
  ...
  wss.on("connection", (ws: WSClient) => {
    ...
    ws.on("message", async (data: Buffer) => {
      try {
        ...
        if (message.type !== "join-room" && !ws.participantId) {
          console.log(
            `Blocked message from unregistered client: ${message.type}`
          );
          return;
        }
        ...
        switch (message.type) {
          case "join-room": {
            ...
            if (!room) {
              ...
              const activeHosts = existingParticipants.filter(
                (p) =>
                  p.isHost &&
              ...
              if (activeHosts.length === 0) {
                ...
                console.log(
                  `No active host in room ${roomId}, promoting ${participantName} to host`
                );
              }
            }
            ...
            if (!isHost && room.isLocked) {
              ...
              console.log(
                `Participant ${participantName} tried to join locked room ${roomId} - connection will be closed`
              );
              ws.close();
              break;
            }
            ...
            if (isHost) {
              ...
              console.log(
                `Participant ${participantName} requesting to join room ${roomId}`
              );
            }
            break;
          }
          ...
          case "approve-participant": {
            ...
            const requestingParticipant = await storage.getParticipant(
              participantId
            );
            if (!requestingParticipant) {
              console.error(
                `Unknown participant ${participantId} tried to approve - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (requestingParticipant.roomId !== roomId) {
              console.error(
                `Participant ${participantId} tried to approve in wrong room ${roomId} - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (!requestingParticipant.isHost) {
              console.error(
                `Non-host ${participantId} tried to approve participant - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            const targetParticipant = await storage.getParticipant(
              targetParticipantId
            );
            if (!targetParticipant || targetParticipant.roomId !== roomId) {
              console.error(
                `Target participant ${targetParticipantId} not in room ${roomId} - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            await storage.updateParticipant(targetParticipantId, {
            ...
            if (targetClient) {
              ...
              if (room) {
                ...
                if (room.spotlightedParticipantId) {
                  const spotlightedParticipant = await storage.getParticipant(
                    room.spotlightedParticipantId
                  );
                  targetClient.send(
                    JSON.stringify({
                      ...
                      spotlightedParticipantName:
                        spotlightedParticipant?.name || null,
                    })
                  );
                }
              }
            ...
            console.log(
              `Host ${participantId} approved participant ${targetParticipantId} in room ${roomId}`
            );
            break;
          }
          ...
          case "deny-participant": {
            ...
            if (!requestingParticipant) {
              console.error(
                `Unknown participant ${participantId} tried to deny - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (requestingParticipant.roomId !== roomId) {
              console.error(
                `Participant ${participantId} tried to deny in wrong room ${roomId} - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (!requestingParticipant.isHost) {
              console.error(
                `Non-host ${participantId} tried to deny participant - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            console.log(
              `Host ${participantId} denied participant ${targetParticipantId} in room ${roomId}`
            );
            break;
          }
          ...
          case "remove-participant": {
            ...
            if (!requestingParticipant) {
              console.error(
                `Unknown participant ${participantId} tried to remove - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (requestingParticipant.roomId !== roomId) {
              console.error(
                `Participant ${participantId} tried to remove in wrong room ${roomId} - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (!requestingParticipant.isHost) {
              console.error(
                `Non-host ${participantId} tried to remove participant - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (targetParticipant.isHost) {
              console.error(
                `Participant ${participantId} tried to remove host - rejected`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            console.log(
              `Host ${participantId} removed participant ${targetParticipantId} from room ${roomId}`
            );
            break;
          }
          ...
          case "leave-room": {
            ...
            const participant = (await storage.getParticipants(roomId)).find(
          ...
          case "chat-message": {
            ...
            console.log(
              `Chat message in room ${roomId} from ${participantName}`
            );
            break;
          }
          ...
          case "signal": {
            ...
            console.log(
              `Signal from ${participantId} to ${targetId} in room ${roomId}`
            );
            break;
          }
          ...
          case "emoji-reaction": {
            ...
            console.log(
              `Participant ${participantName} sent ${emoji} reaction in room ${roomId}`
            );
            break;
          }
          ...
          case "mute-all": {
            ...
            console.log(
              `Host ${participantId} muted all participants in room ${roomId}`
            );
            break;
          }
          ...
          case "transfer-host": {
            ...
            if (
              ...
              console.log(
                `Unauthorized transfer-host attempt by ${participantId}`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (
              !newHost ||
              newHost.approvalStatus !== "approved" ||
              newHost.roomId !== roomId
            ) {
              console.log(
                `Invalid transfer-host target: ${newHostId} (not in room or not approved)`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (newHostId === participantId) {
              console.log(
                `Host ${participantId} attempted to transfer to self`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            await storage.updateParticipant(newHostId, {
              isHost: true,
              canRecord: true,
            });
            ...
            console.log(
              `Host transferred from ${requester.name} to ${newHost.name} in room ${roomId}`
            );
            break;
          }
          ...
          case "force-disable-audio": {
            ...
            if (!requester || !requester.isHost) {
              console.log(
                `Unauthorized force-disable-audio attempt by ${participantId}`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (!target || target.roomId !== roomId) {
              console.log(
                `Invalid force-disable-audio target: ${targetParticipantId}`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (targetClient && targetClient.readyState === WebSocket.OPEN) {
              ...
              console.log(
                `Host ${participantId} force-disabled audio for ${target.name}`
              );
            }
            break;
          }
          ...
          case "force-disable-video": {
            ...
            if (!requester || !requester.isHost) {
              console.log(
                `Unauthorized force-disable-video attempt by ${participantId}`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (!target || target.roomId !== roomId) {
              console.log(
                `Invalid force-disable-video target: ${targetParticipantId}`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (targetClient && targetClient.readyState === WebSocket.OPEN) {
              ...
              console.log(
                `Host ${participantId} force-disabled video for ${target.name}`
              );
            }
            break;
          }
          ...
          case "grant-recording-permission": {
            ...
            if (!requester || !requester.isHost) {
              console.log(
                `Unauthorized grant-recording-permission attempt by ${participantId}`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            if (!target || target.roomId !== roomId) {
              console.log(
                `Invalid grant-recording-permission target: ${targetParticipantId}`
              );
              ws.send(
                JSON.stringify({
                  type: "error",
            ...
            console.log(
              `Host ${participantId} ${
                newCanRecord ? "granted" : "revoked"
              } recording permission for ${target.name}`
            );
            break;
          }
        }
    ...
    ws.on("close", async () => {
      if (ws.participantId && ws.roomId) {
        const participant = (await storage.getParticipants(ws.roomId)).find(
          (p) => p.id === ws.participantId
        );
